...
- **优先级 2（默认：选净票王）**：**先跑 `python3 refresh_votes.py`**——它用 **REST**（`GET /repos/cissy0802/cissy0802.github.io/discussions`，响应自带 `reactions.+1/-1`）把每个议题对应 giscus 讨论（标题＝`topic:`+id）的**净票（👍−👎，其他表情不计）回写进 `ideas.json` 的 `candidates[].votes`**。然后读 `ideas.json`，**选 `votes` 最高的那条**为本场议题（平票或全 0，取清单里最靠前的）。`slug` 直接用该候选的 `id`。
  - **为什么走 REST、不用 `gh api graphql`**：云端环境通常**没有 `gh` CLI**，且 token 代理**只转发 REST、GraphQL 会被 `403 "GraphQL proxying is not enabled"` 拦截**——这正是过去净票一直读不到、`votes` 恒为 0、票王从不生成的根因。REST 讨论列表已自带 `reactions` 汇总，足够算净票，无需 GraphQL。（取数细节与等价逻辑见 `refresh_votes.py`。）
  - **降级**：`refresh_votes.py` 在拿不到 `GH_TOKEN`/`GITHUB_TOKEN` 或取票失败时**会保留现有 `votes`、绝不清零**；此时就直接取 `ideas.json` `candidates` 里**最靠前**的那条（顺序即人工优先级），辩论照常完整——别因为读不到票就卡住。
