...
请严格以 JSON 返回中英双语六段。**篇幅要求：三段都写充分、别过度精简——和另外两家 AI 的长度相当**（summary 与 insight 各约 200–260 字、advice 约 130–180 字；宁可写满，别只写到下限就收）：
{"summary": "综述(中文，约 200–260 字)：务必『先共识、后分歧』——开头先点出这场最大的共识/共同点(往往最值得带走)，再讲分歧的真正焦点。不要一上来就强调歧义",
 "insight": "独到见解(中文，约 200–260 字)：给出一条你认为最值得加的、与全场不同的观察，并把它讲透（为什么成立、对普通人意味着什么）。可以是被忽略的角度、一个具体反例、一种更好的重构、普通人真正会反驳之处、或一个务实的纠偏——**挑这场最该说的那一条；别为了显得深刻而每次都套同一种『太抽象/太文本/太精英化』的框架批判**；若全场已相当周全，直接说『哪一条最该照做、为什么』也好",
 "advice": "给普通人的可操作建议(中文，约 130–180 字)：结合综述与洞察，给 2~3 条当天或本周就能上手的具体行动。可点名援引本场某位思想家最实用的建议，也可补你自己的；落到具体动作，别空泛",
