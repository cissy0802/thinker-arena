...
- 读 `thinkers.json`（150+ 人名册，每人含 `system` 人设、`cat` 类别）。
...
- **人物图鉴必补（非可选）**：选定阵容后，**每位选中的思想家若在 `profiles.json` 里还没有条目，必须为他补一条**（键＝该人 id），含 `bio/ideas/assessments[{by,text}]/quotes/works/lineage/trivia` **且每个字段都配 `_en`**（**类型别写错**：`quotes`/`works` 是字符串数组、`assessments` 是 `[{by,text}]`、`bio`/`ideas`/`lineage`/`trivia` 是字符串——把字符串塞进数组字段会让**整页图鉴白屏**，`validate.py` 现在会硬卡）（`bio_en`/`ideas_en`/`assessments_en`/`quotes_en`/`works_en`/`lineage_en`/`trivia_en`，英文要地道）。评价与名言只用**真实公认史料，严禁编造**；拿不准的名言改写成「大意 / paraphrased」。**`ideas`（主要思想）要写得详尽、深入浅出——给没有该领域背景的人也能看懂：专业术语随写随解释、善用类比、点出它为何重要 / 与现代的呼应；可分多段（前端按换行 `\n` 自动分段），别堆术语。**（范例见 `vasubandhu` 的 ideas——把唯识从零讲清。）**人物图鉴是通用、可复用的介绍——必须「对本场议题盲」地写**：想象你不知道这人是为哪场辩论选的，只为「人物百科」写一版通用条目。**判定标准（写完自检）：把这条图鉴原样搬到该人参与的任何另一场辩论，都同样贴切、读不出本场议题的影子。**这条铁律不只管行文（别写「这一框架直击本题」「正好回应了金钱/幸福…」之类），更要管**取材**——最常见、最隐蔽的 overfit 是「挑那条正好呼应本场的料」：
  ...
  - 该人在本场的立场、与本题的关联，**一律只写进 `casting[].reason`/`reason_en`**，图鉴里一个字都不提。否则辩论页点选角条头像跳进图鉴是空的「待补充」，或得到一版被本场议题污染、换个场景就别扭的介绍。
  - **`reason`/`reason_en` 只写"他在本题里持什么立场、带来什么视角"，绝不写任何后台选角理由**——`首次登场`/`从未登场`/`名册中未上场`/`冷板凳`/`多样性`/`换血`/`优先启用`/`为了平衡阵容`/`选他是为了…` 这类和辩论本身无关的话，一个都不许进（它们只属于 IDEAS.md 置顶行的 ⚑ 选角提示，那是给你自己看的 backstage）。读者看到的介绍，读起来必须像"这个人对这道题怎么想"，而不是"我为什么把这个人排进来"。**也尽量用陈述句直接以该人身份写（"他把 X 立到最硬""他第一个开口"），少用"请他来/让他去/由他来"这种暴露选角口吻的祈使句。**
...
## 3. 三方 AI 收尾（每场必有）
- `summaries` 三条：`{ "ai":"claude|gpt|gemini", "engine", "engine_en", "summary", "insight", "advice", "summary_en", "insight_en", "advice_en" }`（**双语 `_en` 全必填**，英文地道；`engine_en` 是 engine 的英文版，如 `Opus 5 · subscription (native)`、降级版 `fallback: Opus role-play (no key)`）。三段：`summary`(综述) + `insight`(独到见解) + `advice`(给普通人的可操作建议)。
...
- **标题要短，长的话写进副标题（硬要求）**：`question` **控制在 20 字以内**（英文 ~10 词），只留一句能一眼看懂的主问句——**别把「——…」的补充说明、括号里的举例、第二三个追问全堆进标题**（踩过坑：标题一路涨到 40–100 字，索引页糊成一片）。真正重要、舍不得丢的补充，写进**可选的顶层 `sub`/`sub_en`（副标题，双语，一句话，~40 字内）**——前端会在索引卡片与辩论页标题下方以小字灰色渲染；不需要就整个省略（`sub` 是可选字段，不写不影响校验）。`debates/index.json` 追加的那条也同样带上 `sub`/`sub_en`。**候选池同理**：`ideas.json` 的 `q` 也按 20 字上限写，补充说明放 `note`/`note_en`（投票页本来就把 `note` 当副标题渲染）；**合并近义候选时，把新角度并进 `note`，绝不追加到 `q` 后面**（踩过坑：`q` 被塞成 90–130 字的『（含：…）』）。
...
- **维护 `ideas.json`（投票数据）**（注：候选以 `IDEAS.md` 为人手编辑源、每行尾带隐形 `<!--id-->`；`sync_ideas.py`（GitHub Action 在 IDEAS.md 改动时自动跑）会把 IDEAS.md 候选刷进 `ideas.json`，按 id 保住票/英文/副标题。你两边都改、保持一致即可；**新候选务必在 IDEAS.md 行尾加 `<!--id-->`，并在 `ideas.json` 补 `q_en`/`note`/`note_en` 和 `cat`**（`cat`＝投票页主题分组的语言无关键，取 `work`(职场·组织·领导)/`self`(处世·自我·成长)/`life`(人生·生死·家庭)/`ethics`(伦理·政治·社会)/`meta`(形而上·认识·文明) 里最贴的一个，拿不准用 `other`；标签文案在 `ideas.js` 的 `CATS`，不必另存中英文））：① 把本场议题从 `candidates` 移到 `debated`（带 `q`/`q_en`/`date`/`cat`——`cat` 沿用它在 `candidates` 里的值，投票页据此把『已辩』归到该主题下）；② 其余 `candidates[].votes` 已由第 0 步的 `python3 refresh_votes.py` 从后端回写为最新净票（👍−👎）——可再跑一次刷新；**别手填票数**；③ **读观众提案**——**优先直接打后端、以后端为准**(`GET .../comments?page=thinker-open-questions` 拿观众「提个新议题」的提案，`.../comments?page=topic:<id>` 拿各候选下的讨论；带普通 User-Agent，否则 Cloudflare 会 403)。云端 container 也够得到——只要环境的出网白名单里有 `bigcat-engage.cissychen.workers.dev`（2026-09-03 起已放行）。**只有在打不通时（代理 403＝该域名不在白名单）才退回读仓库里的 `audience_inbox.json`**：那是 GitHub Action `refresh-votes.yml` 每12h 从后端拉好的评论快照（`open_questions`＝新议题提案，`per_topic`＝各候选下的讨论），最多滞后 12 小时。把**好的新议题/新角度**提炼成新 `candidates`（`id`+`q`+`q_en`+`note`/`note_en`+`cat`，votes:0），并同步加到 `IDEAS.md` 候选清单**末尾**（别插队首）——这就是「好评论/提案收录进灵感库」。
...
- **钩子要应辩论而生，别锚定成「每家一条＝3 条」**：把本场真正值得记、又未充分展开的**每一个不同角度**各收成一条钩子，追加到 `IDEAS.md`「各场留下的钩子」（按本议题加小节）。**条数随议题自然浮动（约 3–7 条），别照抄最近几场的条数**——最近几场恰好多是 3 条（每家 `insight` 各抄一条）会诱使你也凑成 3；其实同一个 AI 可以贡献两三条不同角度，某家若实在没有新角度也可以不出。钩子≠把三段 `insight` 各复述一遍，而是把全场（含发言里点到却没深挖的、三家共同指向的）值得带走的开放角度都收下。钩子区是廉价的「候选底稿」，**这里要舍得多记**；「克制、ruthlessly prune」只针对下一步「提炼成正式候选」，别拿来砍钩子本身。此处是**选题挖掘用的中文底稿**（可带维护备注），与上面写进 JSON 的干净双语展示版并存。**提炼成新候选要克制**：只有当某钩子的角度**与已辩议题和现有候选都十分不同**时，才立成新候选 `- [ ]`（并加进 `ideas.json` 的 `candidates`）。**立新候选前必做『近义比对』**：把这个候选的问题逐条对照 `ideas.json` 现有 `candidates`——**只要与某条现有候选是『同一道题的不同措辞』或『同一母题的相邻角度』，就绝不另立**，而是把新角度一句话并进那条现有候选的 `note`（融合，不新增）。宁可池子小而利，别靠"每个钩子都立一条"把池子灌胀（踩过坑：`youth-career-or-travel`↔`youth-career-or-explore` 同题两立、`expert-jargon-wall`↔`who-owns-the-words`、`help-that-humiliates`↔`charity-vs-justice`、`who-underwrites-truth`↔`how-knowledge-becomes-obvious` 三对近义——都是该并未并）。**新候选一律追加到候选清单（`IDEAS.md` 与 `ideas.json` `candidates`）的末尾，绝不插到队首**——队首＝下一场，插队首会让每场都接着辩上一场的衍生题、把已排好的候选一直往下挤、彻底违背求异换血（踩过坑：生育→亲密→第三空间 一路钻同一条线）。若只是对某场的补充/延伸，**只留在钩子区、别塞进候选**——候选池求异、换血，别堆同质题（已辩多为 AI/现代生活类，新候选优先往非 AI、跨学科的母题引，好把冷板凳思想家带出来）。**并每次顺手 ruthlessly prune 候选池（每场必做，不是可选）**：开写本场前，先扫一遍 `ideas.json` `candidates`，把**互为近义/同母题**的成对候选合并成一条（保留措辞更好的、把另一条的独到角度并进其 `note`，删掉多余那条），也删掉与已辩重复、纯补充的题；`IDEAS.md` 与 `ideas.json` 两处同步改、**别记录「已砍」历史**。目标：候选池长期保持精炼、求异，不靠数量堆积。
