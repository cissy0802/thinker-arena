...
- 把三家 `insight` 里值得记的新钩子追加到 `IDEAS.md`「各场留下的钩子」（按本议题加小节）。**提炼成新候选要克制**：只有当某钩子的角度**与已辩议题和现有候选都十分不同**时，才立成新候选 `- [ ]`（并加进 `ideas.json` 的 `candidates`）；若只是对某场的补充/延伸，**只留在钩子区、别塞进候选**——候选池求异、换血，别堆同质题（已辩多为 AI/现代生活类，新候选优先往非 AI、跨学科的母题引，好把冷板凳思想家带出来）。
