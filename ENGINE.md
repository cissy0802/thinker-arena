...
- **commit 前通读一遍（必做、不可省）**：`validate.py` 只查结构与字数，**查不出语言质量**。本场所有新写的中文与英文（辩论 `text`/`vernacular`/`glossary`、三家 AI 收尾、`casting.reason`、`profiles.json` 整套图鉴及其 `_en`）都要亲自从头读一遍，保证：① **中文地道**——没有翻译腔、没有生硬照搬英文语序的句子；② **英文地道**——像母语者写的，不是逐字直译的中式英文；③ **没有奇怪的中英混杂**——中文正文里别夹生英文单词（如把「研习」写成「study」、「分析」写成「analyze」），术语该用中文就用中文，只有约定俗成、带空格的专有名词（`A 级`、`KPI`、`OKR`、`PERMA` 等）才保留英文。读到一处别扭就改一处，改完再 commit。
