...
- **每一轮，每位选中的思想家都要发言**（N 人 × 3 轮 = N×3 帖）。第一人称、有锋芒、可 `@反驳`。
- **篇幅硬要求（防字数棘轮）**：**第 1/2 轮每帖 120–180 字，第 3 轮总结陈词 ≤220 字，钩子每条 ≤100 字**；整场正文控制在 **5200 字以内**、每帖均值 **≤180**。踩过坑：早期每帖 ~137 字，一路漂到 ~200 字（91% 的帖子越线），因为 ENGINE 只写了句没人核的「~100–150 字」，而你每次写时实际参照的是上一场的长度——**和标题从 20 字涨到 100 字是同一个机制**。所以：① **别读最近几场的帖子找『该写多长』的感觉**，按上面的数字写；② 写完超了就**删句，不要改写成更长的句子**——先砍重复的论证、砍第二个例子、砍铺垫，留最锋利的那一刀；③ 短不是缺点，**同一个意思用更少的字说出来才是本事**；④ `validate.py` 会对超线的帖/场/钩子打 WARN(不阻断)，**看到就回去删**，别当背景噪音放过去。
...
- 用 `python3 validate.py debates/<slug>.json` 校验（**必跑、有 ERROR 不许 commit**）：两个 JSON 合法；`thinker`/`reaction key`/`reply_to`/`ai`/`participants` 引用都在册；每轮人人到齐；`glossary` 术语在各自 `text` 里、`glossary_en` 术语在各自 `text_en` 里；**每帖有 `text_en`、每条 summary 有 `summary_en`/`insight_en`、`question_en` 在场**；**每位 `participants` 都在 `profiles.json` 里有条目**；**三家 AI 收尾的 `summary`/`insight`/`advice` 各段中文字数落在 150–250**（上限 250 对三家硬卡；下限 150 只对 `claude` 硬卡，`gpt`/`gemini` 是真实 API 产出、偏短只告警不阻断——别为凑字数给它们注水）。**另有两组 WARN**：①『反锚定』——人数/钩子数与最近 2 场相同、或与上一场参与者重叠 >50%；②『反棚化』（防字数棘轮）——单帖超 200(第1/2轮)/240(第3轮)、每帖均值 >200、正文总字 >5200、钩子单条 >150 或均值 >120。WARN 不阻断 commit，但**看到就该回头换个数/换批人/删掉多出来的字**，别让软约定过拟合成定值、也别让篇幅一场胖过一场。
