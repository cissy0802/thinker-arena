...
def _advice_bold_open(sums):
    """三家 AI 的 advice 有几家一上来就是一句加粗（如 3/3）。
    ENGINE 只要求每段克制地标一处重点，没说标在开头；但『**一句总纲**＋随后展开』
    是最省事的写法，三家齐刷刷这么开，读者看到的就是同一副版式换了内容。
    重点该落在这段话真正的转折处，可以在中段，也可以在收束的那一句。"""
    hit = sum(1 for s in sums if (s.get("advice", "") or "").lstrip().startswith("**"))
    return "%d/%d" % (hit, len(sums)) if sums else "-"
...
def _insight_panorama_open(sums):
    """三家 insight 里有几条以『全场/整场/这场/大家/所有人…』这种全景扫射起手。
    与 insight报幕式 互补：那一维要求后半句还带上『漏/没人/忽略』才算，于是
    「全场都默认…」「整场都在分摊…」这类只站高、不点名盲点的开头照样漏网，而它
    同样是把独到观察压成同一个进入方式（先俯瞰全场，再转折）。N/3 连场不降＝
    换个切入：从一个具体场面、一条反例、一句当事人的原话进去。"""
    pat = re.compile(r"^.{0,8}?(?:整场|全场|这场辩论|本场辩论|所有人|大家都|诸位都|全体)")
    n = sum(1 for s in sums if pat.search((s.get("insight", "") or "").strip()))
    return "%d/%d" % (n, len(sums)) if sums else "-"
...
def _opening_hook_word(posts):
    """全场第一帖是不是用『先…』开的口（先说 / 先把 / 先摆 / 先讲 / 先问 / 首先）。
    这是一处静态规则查不出、也没被上面任何维度覆盖的措辞坍缩：连续六场里有五场
    的开场帖都以『先』字起手（先把这件事变成可以数的 / 先摆事实 / 先把问题摆正 /
    先讲一件让人不舒服的事 / 先把话说准），因为『开场先立一个经验事实』这个好习惯
    连带把句子的头两个字也锁死了。立事实照旧，起手式每场换一个。是/否。"""
    if not posts:
        return "-"
    t = (posts[0].get("text") or "").lstrip("　 ")
    return "是" if t.startswith("首先") or t[:1] == "先" else "否"
...
def _post_question_tail(posts):
    """帖子正文以问号收尾的比例。把一段发言用一句反问收住，是最省力也最容易上瘾的
    收束方式——写多了，整场读起来就是二十几个人轮流把球踢回去，谁都不肯把话说完。
    静态规则查不出（每帖都合法），只能逐场看比例：N/N＝人人以问句退场，接近 0
    ＝全场没人留开放的口子，两头都是定型，健康的样子是随发言性质自然浮动。"""
    if not posts:
        return "-"
    n = sum(1 for p in posts if (p.get("text") or "").rstrip().endswith(("？", "?")))
    return "%d/%d" % (n, len(posts))
...
def _casting_len_spread(d):
    """选角 reason 的长度离散度（最长 ÷ 最短，按中文字数）。前两维照的是选角条的
    开头与结尾，这一维照的是它的体量：若整条选角每个人都写到差不多长，读起来就是
    一张表格填了 N 遍——而人与人对这道题的分量本就不同，有的该三句说完，有的值得
    多给两句。接近 1.0 ＝长度被抹平，2.0 上下才是随内容自然浮动的样子。"""
    cs = d.get("casting") or []
    ns = [_cn(c.get("reason") or "") for c in cs]
    ns = [n for n in ns if n]
    if len(ns) < 2:
        return "-"
    return "%.2f" % (max(ns) / min(ns))
...
def _sub_question_tail(d):
    """副标题(sub)是不是又一个问句。主问句 question 本就是问题，副标题若也一律以
    问号收尾（最近几场清一色如此），索引页读下来就是一串双问句、节拍完全一样。
    副标题同样可以是一句把本场张力点破的陈述——这一维照出的是标题层的单一化。"""
    sub = (d.get("sub") or "").strip()
    if not sub:
        return "-"
    return "问句" if sub.endswith(("？", "?")) else "陈述"
...
def _opening_cat(posts):
    """开场帖发言者所属的名册类别。第一句定调：若每场都先由实证/科学那一路摆事实、
    再让哲人去辩，整个系列的开头就固定成同一种节奏——读者读到第三场就知道第一帖会说什么。
    这一维与『收尾帖类别』对称，同样查不出静态违规，只能逐场比。"""
    if not posts:
        return "-"
    first_round = min(p["round"] for p in posts)
    head = [p for p in posts if p["round"] == first_round]
    return _TH_CAT.get(head[0]["thinker"], "?") if head else "-"
...
def _closing_call_to_action(posts):
    """全场最后一帖是不是以一条给读者的动作收尾（『本周给它留两个小时』『别只问机器』
    『一格就够』『皆作此课』）。第3轮要落到可操作是要求，但『整场的最后一句永远是一条
    待办』是另一回事——读者合上页面时拿到的永远是同一种东西，全场的余味被一条指令收走。
    它不在 _r3_everyday_turn（那只查转场口令、且看的是整轮）、也不在收尾帖类别里，
    静态规则查不出。收束本可以是一个画面、一句判断、一个留给对方的问题。是/否。"""
    if not posts:
        return "-"
    last_round = max(p["round"] for p in posts)
    tail = [p for p in posts if p["round"] == last_round]
    if not tail:
        return "-"
    t = (tail[-1].get("text") or "")[-45:]
    pat = re.compile(r"可以试|不妨|请你|本周|今晚|今天就|从今天|先去|去做|记住|别让|别只|别再"
                     r"|务必|就够|试一试|作此课|留出|留两|你再回头")
    return "是" if pat.search(t) else "否"


def _last_round_order_vs_first(posts):
    """末轮的发言次序与首轮有多像（两轮共同出场者的成对先后一致率，0–1）。
    『轮内照抄顺序』只查完全照搬的整轮，查不出更常见的一种：总结轮把开场的队列
    原封不动又念一遍，只把某一位挪到末尾——读者读到第三轮时，节奏已经预测得到，
    谁压轴、谁先开口全场固定。收束的次序本该由这一场谁该被最后回应决定。0–1，
    越接近 1 越是照着首轮的队列收尾。"""
    first = [p["thinker"] for p in posts if p["round"] == 1]
    last_round = max((p["round"] for p in posts), default=1)
    if last_round == 1:
        return "-"
    last = [p["thinker"] for p in posts if p["round"] == last_round]
    common = [t for t in first if t in last]
    if len(common) < 3:
        return "-"
    ri = {t: i for i, t in enumerate(first)}
    seq = [ri[t] for t in last if t in ri]
    pairs = agree = 0
    for i in range(len(seq)):
        for j in range(i + 1, len(seq)):
            ...
            if seq[i] < seq[j]:
                agree += 1
    return round(agree / pairs, 2) if pairs else "-"


def _advice_overlap(sums):
    """三家 advice 彼此有多像（字符二元组的最大两两重合度，0–1）。静态规则与上面所有
    维度都只看单条 advice 自己的形状，查不出一件更隐蔽的事：三家收尾**给的是同一个
    动作**。本场三家不约而同都落到『列清单＋排班表＋绕开父母直接找兄弟姐妹』，读者
    连着看三遍同一条建议，第三条就白给了。数值连场偏高＝三家在收敛，该让至少一家
    换一个落点（换对象、换时间尺度、换成一条『别做什么』）。"""
    ts = [re.sub(r"[^一-鿿]", "", (s.get("advice", "") or "")) for s in sums]
    gs = [set(t[i:i + 2] for i in range(len(t) - 1)) for t in ts]
    best = 0.0
    for i in range(len(gs)):
        for j in range(i + 1, len(gs)):
            u = gs[i] | gs[j]
            if u:
                best = max(best, len(gs[i] & gs[j]) / len(u))
    return round(best, 3)


def _advice_cites_thinker(d, sums):
    """三家 advice 里有几条点名援引了本场思想家（3/3＝每场收尾都靠点名拼图）。
    点名本身是好事，但三家全都以『某某说的那条』搭骨架，就成了另一种模板。"""
    names = [_TH_NAME.get(i, "") for i in d.get("participants", [])]
    n = 0
    for s in sums:
        a = s.get("advice", "") or ""
        if any(nm and nm in a for nm in names):
            n += 1
    return "%d/3" % n

def _not_x_but_y(posts):
    """有多少帖用了『不是X，是Y』『不在X，在Y』这类对偶否定句。这是一个静态规则
    查不出、却极易全场坍缩的措辞模板：它读起来总像点破了什么，于是每个人、每一轮
    都拿它收束论点，一整场下来九个思想家共用同一副腔调。该让它稀疏——多数论点
    直接说成什么即可，不必先否定一个影子对手。"""
    if not posts:
        return "-"
    pat = re.compile(r"(不是|不在|并非|未必在)[^，。；：！？]{1,30}[，,][^，。；：！？]{0,6}?(而是|而在|是|在)")
    n = sum(1 for p in posts if pat.search(p.get("text") or ""))
    return f"{n}/{len(posts)}"

def _r3_method_announce(posts):
    """总结陈词里有几帖先『报幕』再给建议——『办法很简单』『我给一个测试』『做法很具体』
    『其术甚简』『能给的是一道手续』之类的元话语开场。第3轮要求落到可操作，于是每个人
    都先宣布自己要给方法、再给方法；一整轮读下来是同一个报幕腔，比措辞雷同更难察觉。
    建议该直接长在各人的说话方式里：有人直接给动作，有人夹在论证中间，不必人人先报幕。"""
    r3 = [p for p in posts if p["round"] == 3]
    if not r3:
        return "-"
    pats = ("办法很简单", "其术甚简", "做法很具体", "方法很简单", "一道手续", "一个测试",
            "我给一个", "只给一件", "可以这样做", "具体怎么做", "能做的是", "做得到的是",
            "当持三事", "行法如下", "给一条", "给两条", "给三条")
    n = 0
    for p in r3:
        t = p.get("text") or ""
...
def _r3_triad(posts):
    """总结陈词里有几帖是把建议摆成『三条』。第3轮要求落到可操作，最省事的写法就是
    每个人各列三件事——一整轮读下来九段同一个骨架，比任何措辞雷同都更像模板。
    条数该随各人的说话方式浮动：有人给两条，有人只给一句，有人干脆不列清单。"""
    r3 = [p for p in posts if p["round"] == 3]
    if not r3:
        return "-"
    n = 0
    for p in r3:
        t = p.get("text") or ""
        pat = ("三条" in t or "三件" in t or "行法有三" in t
               or re.search(r"其一.{0,80}其二.{0,80}其三", t)
               or re.search(r"第一.{0,80}第二.{0,80}第三", t)
               or re.search(r"一者.{0,80}二者.{0,80}三者", t))
        if pat:
            n += 1
    return "%d/%d" % (n, len(r3))


def _last_round_concede_open(posts):
    """末轮里有多少帖以『我改了一处 / 我修正一处 / 我仍坚持 / 我仍不承认』这类
    第一人称的『先认一半、再守住立场』公式起手。总结轮认下别人的反驳是好事，可一旦
    每位辩手都用同一副句型开口，八段总结就成了同一个模子倒出来的八次让步表演。
    该让收束的进入方式各不相同：有人直接给结论，有人从一个画面进，有人接着骂。"""
    if not posts:
        return "-"
    last_round = max(p["round"] for p in posts)
    tail = [p for p in posts if p["round"] == last_round]
    pat = re.compile(r"^(?:[^。！？]{0,12}?)(我(?:改|修正|收回|承认|认下|收下|仍坚持|仍不|依然坚持)"
                     r"|吾(?:改|仍|终不|不易其)|我的立场(?:不变|依旧))")
    return "%d/%d" % (sum(1 for p in tail if pat.search(p.get("text") or "")), len(tail))
...
def feats(d):
    ...
    return {
        ...
        # 全场第一帖是否以『先』字起手（最近六场有五场如此＝开场措辞被锁死）
        "开场先字起手": _opening_hook_word(posts),
        # 帖子以问号收尾的比例（反问收束用上瘾＝全场互相踢球，谁都不把话说完）
        "帖尾问句": _post_question_tail(posts),
        ...
        # 副标题是问句还是陈述句（最近几场清一色问句＝标题层的节拍被锁死）
        "副标题问号收尾": _sub_question_tail(d),
        # 收尾帖发言者所属的名册类别。每场都让同一路人（尤其东方古人）说最后一句，
        # 是一处静态规则查不出的收束定型：结尾的分量该随议题给不同传统，别固定归谁。
        "开场帖类别": _opening_cat(posts),
        # 全场最后一句是不是又一条待办（每场都以一条给读者的动作收尾＝收束方式被锁死）
        "末帖动作收尾": _closing_call_to_action(posts),
        "收尾帖类别": _closing_cat(posts),
        # 第1轮是不是照着选角名单的次序念了一遍（每场都『是』＝开场节奏被锁死）
        "首轮序=选角序": _cast_order_is_r1(d),
        # 末轮的次序与首轮有多像（接近 1＝总结轮把开场的队列又念了一遍，见函数注释）
        "末轮序≈首轮序": _last_round_order_vs_first(posts),
        # 末轮有多少帖不接任何人（N/N＝总结轮永远是九段互不相干的独白）
        "末轮断连": _last_round_detached(posts),
        # 末轮有多少帖以『我改了一处／我仍坚持』这类第一人称让步公式起手（见函数注释）
        "末轮认错起手": _last_round_concede_open(posts),
        # 第3轮有几帖把建议摆成『三条』（N/N＝整轮同一个骨架，最容易坍缩的收束模板）
        "末轮三条式": _r3_triad(posts),
        # 第3轮有几帖先用『办法很简单/我给一个测试』这类元话语报幕再给建议（N/N＝整轮同一个报幕腔）
        "末轮方法报幕": _r3_method_announce(posts),
        # 有多少帖靠『不是X，是Y』的对偶否定收束论点（见函数注释；全场高比例＝腔调坍缩）
        "对偶否定句": _not_x_but_y(posts),
        ...
        # 三家 advice 有几条靠点名本场思想家搭骨架（3/3＝收尾建议的固定套路）
        "建议点名援引": _advice_cites_thinker(d, sums),
        ...
        # 三家 advice 的两两最大重合度（见函数注释）：连场偏高＝收尾建议在互相收敛
        "建议雷同度": _advice_overlap(sums),
        # 三家 advice 各有几家以一句加粗起手（N/N＝同一副版式，重点该随内容落点浮动）
        "建议加粗起手": _advice_bold_open(sums),
        # 措辞指纹（开头几字）
...
SCALARS = ["人数","轮数","每轮帖数","钩子数","钩子分布","钩子作者类型","钩子带答案表","选角语气收尾","选角代词起手","选角长度离散","二轮全反一轮","二轮同轮追击","二轮点名起手","二轮火力散布","轮内照抄顺序","首轮全独白","开场先字起手","帖尾问句","答案表位次","钩子问号收尾","副标题问号收尾","开场帖类别","收尾帖类别","末帖动作收尾","首轮序=选角序","末轮序≈首轮序","末轮断连","末轮认错起手","末轮三条式","末轮方法报幕","对偶否定句","表态档集合",
           "用了疑惑","表态覆盖率","带glossary帖数","术语轮次分布","术语加粗","加粗压帖尾","古文帖数","建议点名援引","建议带①②③","帖均加粗处","加粗处分布","总结膨胀比","总结数数起手","总结列清单","总结转日常","收尾建议条数","建议编号体例","建议时间锚","建议雷同度","建议加粗起手","insight报幕式","insight全景起手","同场起手撞车","同场起手撞车(去水)"]
flags = []
